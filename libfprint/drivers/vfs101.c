...
static inline unsigned char _byte(int position, int value)
{
...
{
	if ((_byte(0, vdev->seqnum) == vdev->buffer[0]) &&
		(_byte(1, vdev->seqnum) == vdev->buffer[1]))
		return 0;
	else
		return 1;
}
...
static void async_send(struct fpi_ssm *ssm)
{
	...
	vdev->seqnum++;
	vdev->buffer[0] = _byte(0, vdev->seqnum);
	vdev->buffer[1] = _byte(1, vdev->seqnum);
...
static void vfs_get_print(struct fpi_ssm *ssm, unsigned int param, int type)
{
	...
	/* Prepare data for sending */
	data[type][6] = _byte(0, param);
	data[type][7] = _byte(1, param);
...
static void vfs_set_param(struct fpi_ssm *ssm, unsigned int param, unsigned int value)
{
	...
	/* Prepare data for sending */
	data[6] = _byte(0, param);
	data[7] = _byte(1, param);
	data[8] = _byte(0, value);
	data[9] = _byte(1, value);
...
static void vfs_poke(struct fpi_ssm *ssm, unsigned int addr, unsigned int value, unsigned int size)
{
	...
	data[8] = _byte(2, addr);
	data[9] = _byte(3, addr);
	data[10] = _byte(0, value);
	data[11] = _byte(1, value);
	data[12] = _byte(2, value);
	data[13] = _byte(3, value);
	data[14] = _byte(0, size);
